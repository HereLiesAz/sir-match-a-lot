...
| I4 | The desktop app mixes real audio itself — two decks, a crossfader, a sampler — not a stripped-down room-only controller | done — Phase 4: `PlaybackSession` drives `AudioEngine.deckA`/`deckB`, `mixer.crossfade`, and the eight-pad `sampler`, each wrapped in testable `DeckControl`/`SamplerPadControl` state. Not parity with §C/§D, though: there is no platter, no waveform, and no touch/pointer gesture surface — a deck is loaded by typed path or `DesktopFilePicker`'s native file dialog, and played, cued, and crossfaded through plain buttons and a slider (`Main.kt`'s `DeckPanel`/`CrossfaderPanel`). "Full mixing instrument" here means the audio engine end-to-end, not the gesture-driven platter that §C11 calls "the feature" |

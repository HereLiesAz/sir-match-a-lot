...
class MixPlannerTest {
    ...
    // --- Harmonic proximity filter ---

    @Test
    fun `proximity sorts by Camelot distance from the reference`() {
        val reference = track("ref", camelot = "8A")
        val sorted = MixPlanner.byHarmonicProximity(
            listOf(
                track("distant", camelot = "2A"),
                track("relative", camelot = "8B"),
                track("same", camelot = "8A"),
                track("fifth", camelot = "9A"),
            ),
            reference,
        )
        assertEquals("same", sorted.first().id)
        assertEquals("distant", sorted.last().id)
    }

    @Test
    fun `proximity breaks key ties on tempo closeness`() {
        val reference = track("ref", bpm = 128.0, camelot = "8A")
        val sorted = MixPlanner.byHarmonicProximity(
            listOf(
                track("faroff", bpm = 100.0, camelot = "8A"),
                track("close", bpm = 129.0, camelot = "8A"),
            ),
            reference,
        )
        assertEquals("close", sorted.first().id)
    }

    @Test
    fun `proximity sorts unanalysed tracks last`() {
        val reference = track("ref", camelot = "8A")
        val sorted = MixPlanner.byHarmonicProximity(
            listOf(
                track("unmeasured", bpm = null, camelot = null),
                track("measured", camelot = "9A"),
            ),
            reference,
        )
        assertEquals("measured", sorted.first().id)
        assertEquals("unmeasured", sorted.last().id)
    }

    @Test
    fun `proximity is unchanged without a reference key`() {
        val input = listOf(track("a"), track("b"))
        assertEquals(input, MixPlanner.byHarmonicProximity(input, null))
        assertEquals(input, MixPlanner.byHarmonicProximity(input, track("noKey", camelot = null)))
    }

    // --- Suggestions ---

    @Test
    fun `suggestions are ordered best first and exclude the reference`() {
        val reference = track("ref", bpm = 128.0, camelot = "8A")
        val suggestions = MixPlanner.suggestNext(
            listOf(
                reference,
                track("great", bpm = 128.0, camelot = "8A"),
                track("good", bpm = 128.0, camelot = "9A"),
                track("bad", bpm = 90.0, camelot = "3B"),
            ),
            reference,
        )
        assertTrue("reference must not suggest itself", suggestions.none { it.trackB.id == "ref" })
        assertTrue(suggestions.isNotEmpty())
        assertEquals("great", suggestions.first().trackB.id)
        val scores = suggestions.map { it.overallScore }
        assertEquals(scores.sortedDescending(), scores)
    }

    @Test
    fun `suggestions respect the limit and skip unanalysed tracks`() {
        val reference = track("ref", bpm = 128.0, camelot = "8A")
        val library = (1..10).map { track("t$it", bpm = 128.0, camelot = "8A") } +
            track("unmeasured", bpm = null, camelot = null)
        val suggestions = MixPlanner.suggestNext(library, reference, limit = 3)
        assertEquals(3, suggestions.size)
        assertTrue(suggestions.none { it.trackB.id == "unmeasured" })
    }
}
